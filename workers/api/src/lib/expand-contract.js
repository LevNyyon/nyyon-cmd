// EXPAND · plugin contract doc — knowledge-backed (nyyon-lite layer 5).
//
// The 'plugin-format' knowledge doc is the operator-visible copy of the plugin
// contract (docs/plugin-format.md). The EXPAND agent reads it through the
// read_plugin_contract tool before drafting a manifest, so the contract it
// codes against is the one the operator can see and annotate. The seed below
// is docs/plugin-format.md inlined VERBATIM at build time (regenerate it from
// the file when the contract changes — do not hand-edit the string).
//
// Mirrors lib/planner-persona.js: seed a default on first read, then read at
// runtime. Never throws — a missing/empty doc falls back to the inlined default.

import { readKnowledge, writeKnowledge } from './db.js';

const DOC_SLUG = 'plugin-format';

export const PLUGIN_FORMAT_DEFAULT = `# Nyyon Plugin Format — v2 (canonical)
...
4. **Materialize code** — a Worker cannot load code at runtime:
   - Self-hosted: the reference applier writes \`workers/api/src/plugins/<name>/…\`,
...
// Read the plugin contract doc. Seeds it on first read so it shows up in the
// Knowledge tree and is editable. Never throws.
export async function loadPluginFormatDoc(env) {
  try {
    const doc = await readKnowledge(env, DOC_SLUG);
    if (!doc) {
      await writeKnowledge(env, {
        slug: DOC_SLUG,
        title: 'Nyyon Plugin Format — the contract (v2 / v2.1)',
        body: PLUGIN_FORMAT_DEFAULT,
        parent_slug: 'nyyon-root',
      }).catch(() => {});
      return { slug: DOC_SLUG, title: 'Nyyon Plugin Format — the contract (v2 / v2.1)', body: PLUGIN_FORMAT_DEFAULT, seeded: true };
    }
    const body = String(doc.body || '').trim();
    return { slug: DOC_SLUG, title: doc.title || DOC_SLUG, body: body || PLUGIN_FORMAT_DEFAULT, seeded: false };
  } catch {
    return { slug: DOC_SLUG, title: 'Nyyon Plugin Format — the contract (v2 / v2.1)', body: PLUGIN_FORMAT_DEFAULT, seeded: false };
  }
}
